...
class SmartEmailRemindersMCPServer {
  ...
  private async handleReloadRules(args: any) {
  ...
  private async handleScanEmails(args: any) {
  ...
  private async handleTestRuleSyntax(args: any) {
    const { ruleName } = args;
  ...
  private async handleDebugRuleMatching(args: any) {
  ...
  private async handleShowProcessingLog(args: any) {
      ...
      content: [
        {
          ...
          text: `📊 Processing log (last ${hours}h):\n${logs.map(l =>
            `- ${l.timestamp.toISOString()}: ${l.ruleName} → ${l.status}${l.errorMessage ? ` (${l.errorMessage})` : ''}`
          ).join('\n')}`
        }
      ]
    };
  ...
  private async handlePauseRule(args: any) {
    const { ruleName } = args;
  ...
  private async handleActivateRule(args: any) {
    const { ruleName } = args;
  ...
  private async handleListActiveRules(args: any) {
      ...
      content: [
        {
          ...
          text: `📋 Active rules (${rules.length}):\n${rules.map(r =>
            `- ${r.name}: ${r.providers.join(', ')} → ${r.reminderTemplate.listName}`
          ).join('\n')}`
        }
      ]
    };
  ...
  private async handleProcessSpecificEmail(args: any) {
    const { emailId, provider, dryRun = false } = args;

    const result = await this.emailProcessor.processSpecificEmail(
      emailId,
      provider,
      dryRun
    );
